...
describe('OperationalIntelligence UI Component', () => {
    ...
    it('renders all key metrics cards', () => {
        ...
        expect(screen.getByText(/Retail Transactions/i)).toBeInTheDocument();
        expect(screen.getByText(/Neural Forecast/i)).toBeInTheDocument();
        expect(screen.getByText(/In Store Today/i)).toBeInTheDocument();
        expect(screen.getByText(/Agent Efficiency/i)).toBeInTheDocument();
    });

    it('displays the correct revenue and staff count', () => {
        render(<OperationalIntelligence {...mockProps} />);
 
        // Revenue should be ₹300.00
